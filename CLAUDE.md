...
- **Legal files ship with every release.** `LICENSE.txt` and `URHEBERNACHWEIS.txt` are part of the project and must never be removed or go stale: with every version-bump commit, update the "Stand: <Datum> · Spielversion: <vX.Y.Z>" line in `URHEBERNACHWEIS.txt`. Keep the © year current (on year change) in `LICENSE.txt`, the `index.html` header comment, and the in-game notice (Hilfe → "Über das Spiel"). Never strip copyright notices; the README "Urheberrecht & Lizenz" section references both files.
